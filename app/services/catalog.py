...
class DynamicCatalogService:
    ...
    async def get_theme_based_catalogs(
        ...
        # 3. Generate Rows
        # Titles already claimed by an earlier content type. Movie and series are
        # generated by separate LLM calls that cannot see each other, so without this
        # both can independently produce the same row name.
        ...
        async def _generate_for_type(media_type: str, genres: list[int]):
            ...
            try:
                catalogs = await self.row_generator.generate_rows(
                    ...
                    token=token,
                    avoid_titles=set(claimed_titles),
                )
